...
export default function ProfileHeader({ profile, itemCount, copied, onShare, isFollowing, onToggleFollow, isLoggedIn }: ProfileHeaderProps) {
  return (
    <aside className="w-full lg:sticky lg:top-24 space-y-6">
      ...
      <div className="bg-white border border-zinc-200/90 shadow-sm relative group overflow-hidden rounded-sm transition-all duration-300 hover:shadow-md">

        {/* Banner Header in Sidebar */}
        <div className="relative h-32 sm:h-40 border-b border-zinc-200 bg-zinc-50 overflow-hidden select-none">
          {profile.banner_url ? (
            <img
        ...
        <div className="p-6 sm:p-8 relative pt-0 z-10">

          {/* Avatar Image */}
          <div className="relative -mt-12 sm:-mt-14 mb-4 inline-block">
            <div className="relative w-20 h-20 sm:w-28 sm:h-28 border-2 border-white bg-white z-10 shadow-md overflow-hidden rounded-sm group-hover:border-zinc-900 transition-colors">
              <img
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-extrabold text-zinc-950 tracking-tight">{profile.display_name}</h1>
              {profile.is_verified && <SealCheck className="h-5 w-5 text-black shrink-0" weight="fill" />}
            </div>
          ...
          {/* Bio statement */}
          {profile.bio && (
            <p className="text-xs sm:text-sm text-zinc-600 font-medium leading-relaxed mt-3.5 border-l-2 border-black pl-3">
              {profile.bio}
            </p>
          )}
          ...
          <div className="grid grid-cols-2 gap-2 mt-5 py-3 border-y border-zinc-200 text-[10px] font-bold capitalize tracking-wider">
            <div className="bg-zinc-50 p-2.5 border border-zinc-200/80 text-left rounded-sm">
          ...
          <div className="mt-4 space-y-2 text-[11px] font-bold text-zinc-500">
            {profile.location && (
              <div className="flex items-center gap-2">
                <MapPin className="h-3.5 w-3.5 text-zinc-950 shrink-0" />
                <span className="truncate">{profile.location}</span>
              </div>
            )}
            {profile.skills && (
              <div className="flex items-center gap-2">
                <Code className="h-3.5 w-3.5 text-zinc-950 shrink-0" />
                <span className="truncate">{profile.skills}</span>
              </div>
            )}
            {profile.curator_score != null && (
              <div className="flex items-center gap-2">
                <Compass className="h-3.5 w-3.5 text-zinc-950 shrink-0" />
          ...
          {/* Action Buttons */}
          {(() => {
            const canFollow = isLoggedIn && !profile.is_own_profile;
            return (
              <div className={`mt-6 grid grid-cols-1 ${canFollow ? "sm:grid-cols-2 lg:grid-cols-1" : ""} gap-2.5`}>
                <button
                  onClick={onShare}
        ...
        {/* Card Footer metadata line */}
        <div className="px-5 sm:px-6 py-2.5 bg-zinc-50 border-t border-zinc-200 flex justify-between items-center text-[9px] font-bold capitalize tracking-wider text-zinc-400">
